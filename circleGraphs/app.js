...
var width = 960;
var height = 500;
var knitString = "[31,30,28,26,24,22,20,18,16,14,12,10,8,6,4,2]10";
var instructions = [];
  ...
  for (var i=0;i<instructions.length;i++){
    for (var j=0;j<instructions[i][1];j++){
      for (var k=0;k<instructions[i][0].length;k++){
        links.push({source:nodes[pointer%numNodes],target:nodes[(pointer+(instructions[i][0][k]*1))%numNodes], linkLength:5});
        pointer++;
      }
    }
  }
